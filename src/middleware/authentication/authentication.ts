 ...
 */
function shouldIncludeAuth(url: string, includePatterns?: (RegExp | string)[]): boolean {
 ...
 * ```
 */
export function createAuthenticationMiddleware(options: AuthenticationOptions): FetchMiddleware {
  const {
    tokenProvider,
  ...
  return async (request, next) => {
    ...
    if (shouldSkipAuth(url, skipPatterns) || !shouldIncludeAuth(url, includePatterns)) {
      return next(request);
    }
