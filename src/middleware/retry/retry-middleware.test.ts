...
describe('Retry Middleware - Basic Configuration Tests', () => {
  ...
  describe('useRetry', () => {
    it('should add retry middleware to client', () => {
      ...
      const useResponseMiddlewareSpy = vi.spyOn(
        client,
      ...
      expect(useResponseMiddlewareSpy).toHaveBeenCalledWith(
        expect.any(Function),
      );
    });
  ...
  describe('Retry Conditions', () => {
    ...
    it('should test different retry strategies', () => {
      const fixedMiddleware = createRetryMiddleware({
        strategy: 'fixed',
        delay: 1000,
      });
      const linearMiddleware = createRetryMiddleware({
        strategy: 'linear',
        delay: 1000,
      });
      const exponentialMiddleware = createRetryMiddleware({
        strategy: 'exponential',
        delay: 1000,
      });
